...
受 Alpha Arena 和 open-nof1.ai 项目的启发，本项目在 open-nof1.ai 的基础上进行了全面改进（提示词、数据获取、增加了反馈逻辑、优化了 UI 页面和图标显示）。这是一个基于人工智能的自动化加密货币期货交易系统，使用 Next.js 构建，集成了币安期货 API 和 DeepSeek AI 模型。未来我们将对算法、提示词、模型、分析方法和交易逻辑进行进一步改进，敬请关注！

## 更新
11.3
- 修改前端中 trade 小数精度不足不显示的问题，并添加内容
- 修改前端 chat 一次输出五条信息的问题，现在只输出一条信息，节省空间
...
## 交易流程 (Pipeline)
交易逻辑：从官方 API 调取实时市场数据，每三分钟调用一次 DeepSeek LLMs API，大模型经过精心准备输入的提示词 (Prompt)，给出分析之后的策略，调用交易所 API 进行交易。
...
4. 创建新的 API Key
5. 保存 API Key
（最好使用 DeepSeek, OpenRouter 可以不配置）
...
2. **编辑 `.env` 文件（配置数据库，币安四个 key，交易模式和 DeepSeek API key）**
   ...
   **DATABASE_URL="postgresql://trading_user:(your_secure_password)@localhost:5432/nof1"**
   ...
   # 代理配置（在中国大陆访问需要代理并且需要非中非美 ip）
   # ==========================================
   # 如果需要通过代理访问币安 API（clash 需用端口 7890，V2Ray 端口 10809）
   ...
   **BINANCE_LIVE_API_KEY="你的实盘API密钥"
   ...
   MAX_POSITION_SIZE_USDT=5000  # 最大持仓 Maximum position size in USDT (increased for aggressive strategy)
   MAX_LEVERAGE=30  # 最大杠杆 Maximum allowed leverage (increased to 30x for high-yield strategy)
   DAILY_LOSS_LIMIT_PERCENT=20  # 最大日损失限制 Daily loss limit as percentage of capital (20% for aggressive trading)
...
- 确认 PostgreSQL 服务是否运行（若打开电脑后发现 pgAdmin 的数据库前无法连接（红色叉），需要打开服务-postgresql 右键启动）
- 验证数据库密码和数据库名称是否正确
...
**若要切换模拟盘和实盘策略**：
- 只需在 .env 中配置好两种 API 之后，将 TRADING_MODE live 和 dry-run 对调即可

### 数据库管理

只需下载之后在 pgAdmin 可视化界面中查看
<img width="1518" height="1143" alt="image" src="https://github.com/user-attachments/assets/1ec01f5d-ddc5-4922-911a-5981a02c7acb" />
在右上的 dashboard 中看到数据波动，在 SQL 中可利用数据库语言进行查询
