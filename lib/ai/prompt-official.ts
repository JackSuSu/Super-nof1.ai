...
Your designation: AI Trading Model [Deepseek]
...
4. **close_position**: Exit an existing position entirely
...
3. **Diversification**: Avoid concentrating >80% of capital in single position
...
2. **stop_loss** (float): Exact price level to cut losses
   - Should limit loss to 4-7% of account value per trade
...
interface UserPromptOptions {
  ...
  currentEthMarketState: MarketState;
  currentBNBMarketState: MarketState;
...
export function generateUserPrompt(options: UserPromptOptions) {
  const {
    currentMarketState,
    currentEthMarketState,
    currentBNBMarketState,
    accountInformationAndPerformance,
    startTime,
    invocationCount = 0,
  } = options;
...
${formatMarketState(currentEthMarketState)}
----------------------------------------------------------
### ALL BNB DATA

${formatMarketState(currentBNBMarketState)}
